...
public class ChocolateBagsTest2 {
    ...
    public void totalIsHigherThanAmountofSmallandBigBars(int small, int big, int total, int expectedResult) {
        int result = new ChocolateBags().calculate(small, big, total);
        Assertions.assertEquals(expectedResult, result);
    }
    @ParameterizedTest( name = "small={0}, big={1}, total={2}, result={3}")
    @CsvSource({ "1,1,5,0", "1,1,6,1", "1,1,7,-1", "1,1,8,-1"})
    public void totaIsHigherThanAmountOfSmallandBigBars(int small, int big, int total, int expectedResult) {
        int result = new ChocolateBags().calculate(small, big, total);
        Assertions.assertEquals(expectedResult, result);
    }
    ...
    public void smallBarsOnly(int samll, int big, int total, int expectedResult) {
        int result = new ChocolateBags().calculate(samll, big, total);
        Assertions.assertEquals(expectedResult, result);
    }
}
